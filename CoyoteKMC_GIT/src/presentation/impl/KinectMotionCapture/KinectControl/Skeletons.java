...
public class Skeletons implements Serializable{
//  private static final String HEAD_FNM = "gorilla.png";
    ...
    private SkeletonUser skeletonUser;
    private ArrayList<PointJoint> listPoint = new ArrayList<PointJoint>();
...
//    headImage = loadImage(HEAD_FNM);
    ...
    private BufferedImage loadImage(String fnm) // load the image from fnm
    {
    ...
    private void configure() /* create pose and skeleton detection capabilities for the user generator, 
    ...
    protected SkeletonUser updateJoints(int userID) // update all the joints for this userID in userSkels
    {
        ...
        //cria esqueleto do usuário
     
        skeletonUser = new SkeletonUser(listPoint, userID);
      //  listPoint.removeAll(listPoint);
        return skeletonUser;
    }  // end of updateJoints()

    public SkeletonUser getSkeletonUser(int idUser) {
        return updateJoints(idUser);
    }

    public SkeletonUser getSkeletonUser() {
        update();
        return skeletonUser;
    }
    ...
    private void updateJoint(HashMap<SkeletonJoint, SkeletonJointPosition> skel,
            int userID, SkeletonJoint joint) /* update the position of the specified user's joint by 
     ...
     */ {
        try {
            ...
            skel.put(joint, jPos);
            //adiciona junta na lista
            listPoint.add(new PointJoint(joint, jPos.getPosition()));
        } catch (StatusException e) {
            System.out.println(e);
        }
    }  // end of updateJoint()
    ...
    // -------------------- drawing --------------------------------
    public Graphics2D draw(Graphics2D g2d) // draw skeleton of each user, with a head image, and user status
    {
    ...
    private void setLimbColor(Graphics2D g2d, int userID) /* use the 'opposite' of the user ID color for the limbs, so they
    ...
    private void drawSkeleton(Graphics2D g2d,
            HashMap<SkeletonJoint, SkeletonJointPosition> skel) // draw skeleton as lines (limbs) between its joints;
    ...
    private void drawLine(Graphics2D g2d,
            HashMap<SkeletonJoint, SkeletonJointPosition> skel,
            SkeletonJoint j1, SkeletonJoint j2) // draw a line (limb) between the two joints (if they have positions)
    {
...
/*
    private void drawHead(Graphics2D g2d,
            HashMap<SkeletonJoint, SkeletonJointPosition> skel) // draw a head image rotated around the z-axis to follow the neck-->head line
    {
        if (headImage == null) {
            return;
        }
...
/*
    ...
    private void drawUserStatus(Graphics2D g2d, int userID) throws StatusException // draw user ID and status on the skeleton at its center of mass (CoM)
    {
